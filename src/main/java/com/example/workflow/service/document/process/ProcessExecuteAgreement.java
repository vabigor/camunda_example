...
import com.example.workflow.service.document.validate.ProcessValidateBase;
...
@Component
public class ProcessExecuteAgreement implements ProcessExecute{
    ...
    private final ProcessValidateBase processValidateBase;

    @Autowired
    public ProcessExecuteAgreement(TaskService taskService,
                                   DocumentServiceDao documentServiceDao, ProcessServiceDao processServiceDao,
                                   ProcessTaskServiceDao processTaskServiceDao, ProcessValidateBase processValidateBase) {
        ...
        this.processValidateBase = processValidateBase;
    }
    ...
    public void execute(UUID docId, DocumentProcessResult result, CustomUser user) {
        ...
        if (processTask.getExecutionCount()==processTask.getExecuteCount() && result.isNext()){
            ...
            System.out.println("Agreement execute");
        }
    }

    @Override
    public boolean validate(DelegateTask delegateTask) {
        processValidateBase.validate(delegateTask);
        return false;
    }

    @Override
    public DocumentStatus getType() {
        return DocumentStatus.AGREEMENT;
    }
}
